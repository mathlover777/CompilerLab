...
void RegExpToNFA(RegExp* r,int regexpsize,NFA* a)//INPUT REGULAR EXPRSSION IN POSTFIX FORM
{
	...
	for(i=0;i<regexpsize;i++)
	{
		...
		else if(r->Expression[i]==OADD)
		{
			...
			printf("\nIn Add before alloc temp1 MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			//temp1=allocNFA();
			//init(temp1);
			//temp2=allocNFA();
			//init(temp2);
			temp3=allocNFA();
			init(temp3);
			printf("\nIn Add after alloc MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			temp1=NFAstack_top(process);
			NFAstack_pop(process);	
			if(isNFAStackEmpty(process)) 
			{
			...
			//countfree++;
			freeNFANonptr(&temp2);
			printf("\nIn Add after add MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			//countfree++;
			//freeNFA(temp3);
		}
		...
		{
			printf("\nBefore temp3 MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			temp3=allocNFA();
			init(temp3);
			// printf("ENCOUNTERED Symbol :%c \n",r->Expression[i]);
			printf("\nAfter init MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			singleNFA(r->Expression[i],temp3);
			printf("\nAfter singleNFA MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			// printf("PRINTING TEMP3\n");
			//printNFA(temp3);
			NFAstack_push(process,*temp3);
			printf("\nAFTER NFAstack MALLOC::%d     FREE::%d \n ",countmalloc, countfree);
			//freeNFA(temp3);		
		}
		//freeNFA(temp1);
		//freeNFA(temp2);
...
int lineToTokens(const char *line,const char *symtab,const char *outfile,DFA *N,int nfaCount){
	int lbegin=0;
	int bestmatch;
	int match1,match2;
	int Code[MAXDFACOUNT];
	int End[MAXDFACOUNT];
	...
	while(1){
		for(int i=0;i<nfaCount;i++){
			simulate_DFA(&N[i],line,lbegin,&End[i],&Attrib[i]);
		}
...
void TokenCreator(const char *source,const char *symtab,const char *outfile,DFA *N,int nfaCount){
	printf("\nFILENAME = '%s'\n",source);
