...
;
stmtList:
	M01 stmtList M1 stmt
	{
		//printf("\nLABEL at stmtList1 = {%s}",$<STMT>3.next);
		initCode(&($$.C));
		//printf("\nGOING TO CONCAT!!");
		concatCode(&($$.C),&($2).C,&($4).C);
		char Line[100];
		sprintf(Line,"%s:",$<STMT>3.next);
		//printf("\nHERE GONE LINE = {%s}??\n",Line);
		addCode(&($$.C),Line);
		//printf("\nNO\n");
	}
...
M1:
	{
		//getNewLabel($$.next);
		strcpy($$.next,$<STMT>-2.next);
		//printf("\nLABEL FROM M1 : {%s}",$$.next);
	}
M2:
	{
		//getNewLabel($$.next);
		strcpy($$.next,$<STMT>-1.next);
